...
export default function ItemPage() {
  ...
  const [loading, setLoading] = useState(true); // <— control showing skeleton
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      setLoading(true);              // show skeleton on first load & when id changes
      setItemCollection({});         // optional: clear old data
      setTimeLeft(0);
      try {
  ...
  if (loading) return <ItemPageSkeleton />; // <— use YOUR skeleton UI
